...
import { startLoadingSample, sampleLoadingState, sampleLoadEvents, SampleLoadedEvent, loadBuiltInSamples, toNameMap, Config, getDrumWave, drawNoiseSpectrum, getArpeggioPitchIndex, performIntegralOld, getPulseWidthRatio, effectsIncludeTransition, effectsIncludeChord, effectsIncludePitchShift, effectsIncludeDetune, effectsIncludeVibrato, effectsIncludeNoteFilter, effectsIncludeDistortion, effectsIncludeBitcrusher, effectsIncludePanning, effectsIncludeChorus, effectsIncludeEcho, effectsIncludeReverb, /*effectsIncludeNoteRange,*/ effectsIncludeRingModulation, effectsIncludeGranular, calculateRingModHertz } from "./SynthConfig.js";
import { EditorConfig } from "./editor/EditorConfig.js";
import { scaleElementsByFactor, inverseRealFourierTransform } from "./FFT.js";
...
class HarmonicsWaveState {
    ...
    getCustomWave(settings, instrumentType) {
        ...
        const harmonicsRendered = (instrumentType == 7 /* InstrumentType.pickedString */) ? Config.harmonicsRenderedForPickedString : Config.harmonicsRendered;
...
export class FilterControlPoint {
    ...
    getLinearGain(peakMult = 1.0) {
        ...
        const neutral = (this.type == 2 /* FilterType.peak */) ? 0.0 : -0.5;
...
export class FilterSettings {
    ...
    convertLegacySettings(legacyCutoffSetting, legacyResonanceSetting, legacyEnv) {
        ...
        const envDecays = (legacyEnv.type == 5 /* EnvelopeType.flare */ || legacyEnv.type == 6 /* EnvelopeType.twang */ || legacyEnv.type == 10 /* EnvelopeType.decay */ || legacyEnv.type == 1 /* EnvelopeType.noteSize */);
    ...
    fromJsonObject(envelopeObject, format) {
        ...
        if (format == "slarmoosbox") {
            if (envelopeObject["envelope"] == "tremolo2") {
                envelope = Config.newEnvelopes[8 /* EnvelopeType.lfo */];
                isTremolo2 = true;
            }
            ...
            if (Config.envelopes.dictionary[envelopeObject["envelope"]].type == 9 /* EnvelopeType.tremolo2 */) {
                envelope = Config.newEnvelopes[8 /* EnvelopeType.lfo */];
                isTremolo2 = true;
            }
            else if (Config.newEnvelopes[Math.max(Config.envelopes.dictionary[envelopeObject["envelope"]].type - 1, 0)].index > 8 /* EnvelopeType.lfo */) {
...
export class Instrument {
    ...
    setTypeAndReset(type, isNoiseChannel, isModChannel) {
        ...
        this.unison = 0;
        this.stringSustain = 10;
        this.stringSustainType = Config.enableAcousticSustain ? 1 /* SustainType.acoustic */ : 0 /* SustainType.bright */;
        this.clicklessTransition = false;
        this.arpeggioSpeed = 12;
    ...
    convertLegacySettings(legacySettings, forceSimpleFilter) {
        ...
        if (legacyPulseEnv == undefined)
            legacyPulseEnv = Config.envelopes.dictionary[(this.type == 6 /* InstrumentType.pwm */) ? "twang 2" : "none"];
        if (legacyOperatorEnvelopes == undefined)
            legacyOperatorEnvelopes = [Config.envelopes.dictionary[(this.type == 1 /* InstrumentType.fm */) ? "note size" : "none"], Config.envelopes.dictionary["none"], Config.envelopes.dictionary["none"], Config.envelopes.dictionary["none"]];
        ...
        let noteSizeControlsSomethingElse = (legacyFilterEnv.type == 1 /* EnvelopeType.noteSize */) || (legacyPulseEnv.type == 1 /* EnvelopeType.noteSize */);
    ...
    toJsonObject() {
        ...
        else if (this.type == 1 /* InstrumentType.fm */) {
            const operatorArray = [];
        ...
        else if (this.type == 11 /* InstrumentType.fm6op */) {
            const operatorArray = [];
    ...
    fromJsonObject(instrumentObject, isNoiseChannel, isModChannel, useSlowerRhythm, useFastTwoNoteArp, legacyGlobalReverb = 0, jsonFormat = Config.jsonFormat) {
        ...
        if (Array.isArray(instrumentObject["effects"])) {
            ...
            this.effects = (effects & ((1 << 15 /* EffectType.length */) - 1));
        }
        else {
        ...
        this.stringSustainType = Config.enableAcousticSustain ? Config.sustainTypeNames.indexOf(instrumentObject["stringSustainType"]) : 0 /* SustainType.bright */;
        ...
        if (this.type == 1 /* InstrumentType.fm */ || this.type == 11 /* InstrumentType.fm6op */) {
            ...
            for (let j = 0; j < Config.operatorCount + (this.type == 11 /* InstrumentType.fm6op */ ? 2 : 0); j++) {
    ...
    addEnvelope(target, index, envelope, newEnvelopes, start = 0, end = -1, inverse = false, perEnvelopeSpeed = -1, perEnvelopeLowerBound = 0, perEnvelopeUpperBound = 1, steps = 2, seed = 2, waveform = 0 /* LFOEnvelopeTypes.sine */, discrete = false) {
    ...
    getTransition() {
        return effectsIncludeTransition(this.effects) ? Config.transitions[this.transition] :
            (this.type == 10 /* InstrumentType.mod */ ? Config.transitions.dictionary["interrupt"] : Config.transitions.dictionary["normal"]);
    }
    getFadeInSeconds() {
        return (this.type == 4 /* InstrumentType.drumset */) ? 0.0 : Synth.fadeInSettingToSeconds(this.fadeIn);
    }
    getFadeOutTicks() {
        return (this.type == 4 /* InstrumentType.drumset */) ? Config.drumsetFadeOutTicks : Synth.fadeOutSettingToTicks(this.fadeOut);
    }
    getChord() {
...
export class Song {
    ...
    toBase64String() {
        ...
        buffer.push(107 /* SongTagCode.key */, base64IntToCharCode[this.key], base64IntToCharCode[this.octave - Config.octaveMin]);
        ...
        for (let channelIndex = 0; channelIndex < this.getChannelCount(); channelIndex++) {
            for (let i = 0; i < this.channels[channelIndex].instruments.length; i++) {
                ...
                buffer.push(118 /* SongTagCode.volume */, base64IntToCharCode[(instrument.volume + Config.volumeRange / 2) >> 6], base64IntToCharCode[(instrument.volume + Config.volumeRange / 2) & 0x3f]);
                buffer.push(117 /* SongTagCode.preset */, base64IntToCharCode[instrument.preset >> 6], base64IntToCharCode[instrument.preset & 63]);
                ...
                if (instrument.type == 0 /* InstrumentType.chip */) {
                    ...
                    for (let o = 0; o < (instrument.type == 11 /* InstrumentType.fm6op */ ? 6 : Config.operatorCount); o++) {
                    ...
                    if (Config.stringSustainRange > 0x20 || 2 /* SustainType.length */ > 2) {
    ...
    fromBase64String(compressed, jsonFormat = "auto") {
        ...
        else if (variantTest == 0x73 || variantTest == 0x70) { //"s"
                ...
                case 84 /* SongTagCode.startInstrument */:
                    {
                        ...
                        let instrumentType = validateRange(0, 12 /* InstrumentType.length */ - 1, base64CharCodeToInt[compressed.charCodeAt(charIndex++)]);
                ...
                case 73 /* SongTagCode.stringSustain */:
                    {
                        ...
                        instrument.stringSustainType = Config.enableAcousticSustain ? clamp(0, 2 /* SustainType.length */, sustainValue >> 5) : 0 /* SustainType.bright */;
                    }
                    break;
                ...
                case 113 /* SongTagCode.effects */:
                    {
                        const instrument = this.channels[instrumentChannelIterator].instruments[instrumentIndexIterator];
                        if ((beforeNine && fromBeepBox) || ((fromJummBox && beforeFive) || (beforeFour && fromGoldBox))) {
                            instrument.effects = (base64CharCodeToInt[compressed.charCodeAt(charIndex++)] & ((1 << 15 /* EffectType.length */) - 1));
                            if (legacyGlobalReverb == 0 && !((fromJummBox && beforeFive) || (beforeFour && fromGoldBox))) {
                        ...
                        // Clamp the range.
                        instrument.effects &= (1 << 15 /* EffectType.length */) - 1;
                    }
                    break;
                ...
                case 69 /* SongTagCode.envelopes */:
                    {
                        ...
                        if ((beforeNine && fromBeepBox) || (beforeFive && fromJummBox) || (beforeFour && fromGoldBox)) {
                            ...
                            for (let i = 0; i < envelopeCount; i++) {
                                ...
                                let steps = 2;
                                let seed = 2;
                                let waveform = 0 /* LFOEnvelopeTypes.sine */;
                            ...
                            if (isModChannel) {
                                ...
                                for (let instrumentIndex = 0; instrumentIndex < channel.instruments.length; instrumentIndex++) {
                                    ...
                                    for (let mod = 0; mod < Config.modCount; mod++) {
                                        ...
                                        if (jumfive && Config.modulators[instrument.modulators[mod]].associatedEffect != 15 /* EffectType.length */) {
                            ...
                            for (let i = 0; i < this.patternsPerChannel; i++) {
                                ...
                                while (curPart < this.beatsPerBar * Config.partsPerBeat + (+isModChannel)) {
                                    ...
                                    if (!useOldShape && !newNote) {
                                        ...
                                        let pitch;
                                        let pitchCount = 0;
                                        const pitchBends = []; // TODO: allocate this array only once! keep separate length and iterator index. Use Deque?
...
Song._variant = 0x70; //"p" ~ pulseboxx!!!!!
class PickedString {
    constructor() {
        this.delayLine = null;
        this.allPassG = 0.0;
    ...
    update(synth, instrumentState, tone, stringIndex, roundedSamplesPerTick, stringDecayStart, stringDecayEnd, sustainType) {
        ...
        const register = sustainType == 1 /* SustainType.acoustic */ ? 0.25 : 0.0;
        ...
        const brightnessType = sustainType == 0 /* SustainType.bright */ ? 0 /* PickedStringBrightnessType.bright */ : 1 /* PickedStringBrightnessType.normal */;
        if (brightnessType == 0 /* PickedStringBrightnessType.bright */) {
            ...
            const cornerHardness = Math.pow(brightnessType == 1 /* PickedStringBrightnessType.normal */ ? 0.0 : 1.0, 0.25);
...
class EnvelopeComputer {
    constructor( /*private _perNote: boolean*/) {
        ...
        const length = 56 /* EnvelopeComputeIndex.length */;
    ...
    computeEnvelopes(instrument, currentPart, tickTimeStart, tickTimeStartReal, secondsPerTick, tone, timeScale, instrumentState, synth, channelIndex, instrumentIndex, perNote) {
        ...
        for (let envelopeIndex = 0; envelopeIndex <= instrument.envelopeCount; envelopeIndex++) {
            ...
            const pitch = (envelope.type == 2 /* EnvelopeType.pitch */) ? this.computePitchEnvelope(instrument, envelopeIndex, (this.startPinTickPitch || this.getPitchValue(instrument, tone, instrumentState, true))) : 0;
...
class InstrumentState {
    constructor() {
        ...
        this.releasedTones = new Deque(); // Tones that are in the process of fading out after the corresponding notes ended.
        this.liveInputTones = new Deque(); // Tones that are initiated by a source external to the loaded song data.
        this.type = 0 /* InstrumentType.chip */;
    ...
    compute(synth, instrument, samplesPerTick, roundedSamplesPerTick, tone, channelIndex, instrumentIndex) {
        ...
        */
        ...
        if (usesGranular) {
            ...
            for (let iterations = 0; iterations < Math.ceil(Math.random() * Math.random() * 10); iterations++) { //dirty weighting toward lower numbers
                ...
                if (this.granularGrainsLength < this.granularMaximumGrains && Math.random() <= granularChance) { //only create a grain if there's room and based on grainFreq
                    ...
                    if (Config.granularEnvelopeType == 0 /* GranularEnvelopeType.parabolic */) {
                        grain.initializeParabolicEnvelope(grain.maxAgeInSamples, 1.0);
                    }
                    else if (Config.granularEnvelopeType == 1 /* GranularEnvelopeType.raisedCosineBell */) {
        ...
        if (usesDistortion) {
            ...
            const distortionSliderStart = Math.min(1.0, envelopeStarts[42 /* EnvelopeComputeIndex.distortion */] * useDistortionStart / (Config.distortionRange - 1));
            const distortionSliderEnd = Math.min(1.0, envelopeEnds[42 /* EnvelopeComputeIndex.distortion */] * useDistortionEnd / (Config.distortionRange - 1));
        ...
        if (usesBitcrusher) {
            let freqSettingStart = instrument.bitcrusherFreq * Math.sqrt(envelopeStarts[44 /* EnvelopeComputeIndex.bitcrusherFrequency */]);
            let freqSettingEnd = instrument.bitcrusherFreq * Math.sqrt(envelopeEnds[44 /* EnvelopeComputeIndex.bitcrusherFrequency */]);
            // Check for freq crush mods
            if (synth.isModActive(Config.modulators.dictionary["freq crush"].index, channelIndex, instrumentIndex)) {
                freqSettingStart = synth.getModValue(Config.modulators.dictionary["freq crush"].index, channelIndex, instrumentIndex, false) * Math.sqrt(envelopeStarts[44 /* EnvelopeComputeIndex.bitcrusherFrequency */]);
                freqSettingEnd = synth.getModValue(Config.modulators.dictionary["freq crush"].index, channelIndex, instrumentIndex, true) * Math.sqrt(envelopeEnds[44 /* EnvelopeComputeIndex.bitcrusherFrequency */]);
            }
            let quantizationSettingStart = instrument.bitcrusherQuantization * Math.sqrt(envelopeStarts[43 /* EnvelopeComputeIndex.bitcrusherQuantization */]);
            let quantizationSettingEnd = instrument.bitcrusherQuantization * Math.sqrt(envelopeEnds[43 /* EnvelopeComputeIndex.bitcrusherQuantization */]);
            // Check for bitcrush mods
            if (synth.isModActive(Config.modulators.dictionary["bit crush"].index, channelIndex, instrumentIndex)) {
                quantizationSettingStart = synth.getModValue(Config.modulators.dictionary["bit crush"].index, channelIndex, instrumentIndex, false) * Math.sqrt(envelopeStarts[43 /* EnvelopeComputeIndex.bitcrusherQuantization */]);
                quantizationSettingEnd = synth.getModValue(Config.modulators.dictionary["bit crush"].index, channelIndex, instrumentIndex, true) * Math.sqrt(envelopeEnds[43 /* EnvelopeComputeIndex.bitcrusherQuantization */]);
            }
        ...
        let mixVolumeEnd = mainInstrumentVolume /** envelopeEnds[  InstrumentAutomationIndex.mixVolume]*/;
        ...
        if (usesPanning) {
            const panEnvelopeStart = envelopeStarts[41 /* EnvelopeComputeIndex.panning */] * 2.0 - 1.0;
            const panEnvelopeEnd = envelopeEnds[41 /* EnvelopeComputeIndex.panning */] * 2.0 - 1.0;
        ...
        if (usesChorus) {
            const chorusEnvelopeStart = envelopeStarts[45 /* EnvelopeComputeIndex.chorus */];
            const chorusEnvelopeEnd = envelopeEnds[45 /* EnvelopeComputeIndex.chorus */];
        ...
        if (usesRingModulation) {
            ...
            let useRingModEnvelopeStart = envelopeStarts[49 /* EnvelopeComputeIndex.ringModulation */];
            let useRingModEnvelopeEnd = envelopeEnds[49 /* EnvelopeComputeIndex.ringModulation */];
            ...
            let useRingModHzEnvelopeStart = envelopeStarts[50 /* EnvelopeComputeIndex.ringModulationHz */];
            let useRingModHzEnvelopeEnd = envelopeEnds[50 /* EnvelopeComputeIndex.ringModulationHz */];
        ...
        if (usesEcho) {
            const echoSustainEnvelopeStart = envelopeStarts[46 /* EnvelopeComputeIndex.echoSustain */];
            const echoSustainEnvelopeEnd = envelopeEnds[46 /* EnvelopeComputeIndex.echoSustain */];
            ...
            const echoDelayEnvelopeStart = envelopeStarts[55 /* EnvelopeComputeIndex.echoDelay */];
            const echoDelayEnvelopeEnd = envelopeEnds[55 /* EnvelopeComputeIndex.echoDelay */];
        ...
        if (usesReverb) {
            const reverbEnvelopeStart = envelopeStarts[47 /* EnvelopeComputeIndex.reverb */];
            const reverbEnvelopeEnd = envelopeEnds[47 /* EnvelopeComputeIndex.reverb */];
...
export class Synth {
    ...
    determineInvalidModulators(instrument) {
        ...
        for (let mod = 0; mod < Config.modCount; mod++) {
            ...
            for (let i = 0; i < tgtInstrumentList.length; i++) {
                ...
                // Check effects
                if (!((Config.modulators[instrument.modulators[mod]].associatedEffect != 15 /* EffectType.length */ && !(tgtInstrument.effects & (1 << Config.modulators[instrument.modulators[mod]].associatedEffect)))
    ...
    constructor(song = null) {
        ...
        this.audioProcessCallback = (audioProcessingEvent) => {
            ...
            if (!this.isPlayingSong && performance.now() >= this.liveInputEndTime) {
                ...
                if (this.oscEnabled) {
                    events.raise("oscUpdate", outputDataL, outputDataR);
                }
            }
        };
    ...
    synthesize(outputDataL, outputDataR, outputBufferLength, playSong = true) {
        ...
        while (bufferIndex < outputBufferLength && !ended) {
            ...
            for (let i = bufferIndex; i < runEnd; i++) {
                ...
                if (i === bufferIndex) {
                    events.raise("limiterUpdate", limitedVolume, this.song.outVolumeCap);
                }
            }
            ...
            if (this.tickSampleCountdown <= 0) {
                ...
                for (let channel = 0; channel < this.song.pitchChannelCount + this.song.noiseChannelCount; channel++) {
                    for (let instrumentIdx = 0; instrumentIdx < this.song.channels[channel].instruments.length; instrumentIdx++) {
                        ...
                        const arpEnvelopeStart = envelopeStarts[48 /* EnvelopeComputeIndex.arpeggioSpeed */]; //only discrete for now
    ...
    computeTone(song, channelIndex, samplesPerTick, tone, released, shouldFadeOutFast) {
        ...
        for (let i = 0; i < (instrument.type == 11 /* InstrumentType.fm6op */ ? 6 : Config.operatorCount); i++) {
        ...
        if (effectsIncludePitchShift(instrument.effects)) {
            ...
            const envelopeStart = envelopeStarts[18 /* EnvelopeComputeIndex.pitchShift */];
            const envelopeEnd = envelopeEnds[18 /* EnvelopeComputeIndex.pitchShift */];
        ...
        if (effectsIncludeDetune(instrument.effects) || this.isModActive(Config.modulators.dictionary["song detune"].index, channelIndex, tone.instrumentIndex)) {
            const envelopeStart = envelopeStarts[19 /* EnvelopeComputeIndex.detune */];
            const envelopeEnd = envelopeEnds[19 /* EnvelopeComputeIndex.detune */];
        ...
        if (effectsIncludeVibrato(instrument.effects)) {
            ...
            if (tone.prevVibrato != null) {
                ...
                const vibratoDepthEnvelopeStart = envelopeStarts[20 /* EnvelopeComputeIndex.vibratoDepth */];
            ...
            const vibratoDepthEnvelopeEnd = envelopeEnds[20 /* EnvelopeComputeIndex.vibratoDepth */];
        ...
        if (!effectsIncludeNoteFilter(instrument.effects)) {
            ...
            const noteAllFreqsEnvelopeStart = envelopeStarts[1 /* EnvelopeComputeIndex.noteFilterAllFreqs */];
            const noteAllFreqsEnvelopeEnd = envelopeEnds[1 /* EnvelopeComputeIndex.noteFilterAllFreqs */];
            // Simple note filter
            if (instrument.noteFilterType) {
                const noteFreqEnvelopeStart = envelopeStarts[21 /* EnvelopeComputeIndex.noteFilterFreq0 */];
                const noteFreqEnvelopeEnd = envelopeEnds[21 /* EnvelopeComputeIndex.noteFilterFreq0 */];
                const notePeakEnvelopeStart = envelopeStarts[29 /* EnvelopeComputeIndex.noteFilterGain0 */];
                const notePeakEnvelopeEnd = envelopeEnds[29 /* EnvelopeComputeIndex.noteFilterGain0 */];
                ...
                for (let i = 0; i < noteFilterSettings.controlPointCount; i++) {
                    const noteFreqEnvelopeStart = envelopeStarts[21 /* EnvelopeComputeIndex.noteFilterFreq0 */ + i];
                    const noteFreqEnvelopeEnd = envelopeEnds[21 /* EnvelopeComputeIndex.noteFilterFreq0 */ + i];
                    const notePeakEnvelopeStart = envelopeStarts[29 /* EnvelopeComputeIndex.noteFilterGain0 */ + i];
                    const notePeakEnvelopeEnd = envelopeEnds[29 /* EnvelopeComputeIndex.noteFilterGain0 */ + i];
                    ...
                    tone.noteFilters[i].loadCoefficientsWithGradient(Synth.tempFilterStartCoefficients, Synth.tempFilterEndCoefficients, 1.0 / roundedSamplesPerTick, startPoint.type == 0 /* FilterType.lowPass */);
                    noteFilterExpression *= startPoint.getVolumeCompensationMult();
                }
        ...
        if (instrument.type == 1 /* InstrumentType.fm */ || instrument.type == 11 /* InstrumentType.fm6op */) {
            ...
            const carrierCount = (instrument.type == 11 /* InstrumentType.fm6op */ ? instrument.customAlgorithm.carrierCount : Config.algorithms[instrument.algorithm].carrierCount);
            for (let i = 0; i < (instrument.type == 11 /* InstrumentType.fm6op */ ? 6 : Config.operatorCount); i++) {
                const associatedCarrierIndex = (instrument.type == 11 /* InstrumentType.fm6op */ ? instrument.customAlgorithm.associatedCarrier[i] - 1 : Config.algorithms[instrument.algorithm].associatedCarrier[i] - 1);
                ...
                const freqEnvelopeStart = envelopeStarts[5 /* EnvelopeComputeIndex.operatorFrequency0 */ + i];
                const freqEnvelopeEnd = envelopeEnds[5 /* EnvelopeComputeIndex.operatorFrequency0 */ + i];
                let freqStart;
                let freqEnd;
                ...
                expressionStart *= envelopeStarts[11 /* EnvelopeComputeIndex.operatorAmplitude0 */ + i];
                expressionEnd *= envelopeEnds[11 /* EnvelopeComputeIndex.operatorAmplitude0 */ + i];
                // Check for mod-related volume delta
            ...
            let expressionStart = baseExpression * sineExpressionBoost * noteFilterExpression * fadeExpressionStart * chordExpressionStart * envelopeStarts[0 /* EnvelopeComputeIndex.noteVolume */];
            let expressionEnd = baseExpression * sineExpressionBoost * noteFilterExpression * fadeExpressionEnd * chordExpressionEnd * envelopeEnds[0 /* EnvelopeComputeIndex.noteVolume */];
            ...
            let feedbackStart = feedbackAmplitudeStart * envelopeStarts[17 /* EnvelopeComputeIndex.feedbackAmplitude */];
            let feedbackEnd = feedbackAmplitudeEnd * envelopeEnds[17 /* EnvelopeComputeIndex.feedbackAmplitude */];
            ...
            if (instrument.type == 6 /* InstrumentType.pwm */) {
                ...
                const pulseWidthStart = pulseWidthModStart * envelopeStarts[2 /* EnvelopeComputeIndex.pulseWidth */];
                const pulseWidthEnd = pulseWidthModEnd * envelopeEnds[2 /* EnvelopeComputeIndex.pulseWidth */];
                ...
                const decimalOffsetStart = decimalOffsetModStart * envelopeStarts[37 /* EnvelopeComputeIndex.decimalOffset */];
            ...
            if (instrument.type == 0 /* InstrumentType.chip */ || instrument.type == 9 /* InstrumentType.customChipWave */ || instrument.type == 5 /* InstrumentType.harmonics */ || instrument.type == 7 /* InstrumentType.pickedString */ || instrument.type == 3 /* InstrumentType.spectrum */ || instrument.type == 6 /* InstrumentType.pwm */ || instrument.type == 2 /* InstrumentType.noise */ || instrument.type == 4 /* InstrumentType.drumset */) {
                ...
                const voiceCountExpression = (instrument.type == 7 /* InstrumentType.pickedString */) ? 1 : unisonVoices / 2.0;
                settingsExpressionMult *= unisonExpression * voiceCountExpression;
                const unisonEnvelopeStart = envelopeStarts[4 /* EnvelopeComputeIndex.unison */];
                const unisonEnvelopeEnd = envelopeEnds[4 /* EnvelopeComputeIndex.unison */];
            ...
            if (instrument.type == 8 /* InstrumentType.supersaw */) {
                ...
                const curvedDynamismStart = 1.0 - Math.pow(Math.max(0.0, 1.0 - useDynamismStart * envelopeStarts[38 /* EnvelopeComputeIndex.supersawDynamism */]), 0.2);
                const curvedDynamismEnd = 1.0 - Math.pow(Math.max(0.0, 1.0 - useDynamismEnd * envelopeEnds[38 /* EnvelopeComputeIndex.supersawDynamism */]), 0.2);
                ...
                const spreadSliderStart = Math.max(0, useSpreadStart) * envelopeStarts[39 /* EnvelopeComputeIndex.supersawSpread */];
                const spreadSliderEnd = Math.max(0, useSpreadEnd) * envelopeEnds[39 /* EnvelopeComputeIndex.supersawSpread */];
                ...
                // Saw shape mods
                let useShapeStart = baseShape * envelopeStarts[40 /* EnvelopeComputeIndex.supersawShape */];
                let useShapeEnd = baseShape * envelopeEnds[40 /* EnvelopeComputeIndex.supersawShape */];
                ...
                const shapeStart = useShapeStart * envelopeStarts[40 /* EnvelopeComputeIndex.supersawShape */];
                const shapeEnd = useShapeEnd * envelopeEnds[40 /* EnvelopeComputeIndex.supersawShape */];
                ...
                let pulseWidthStart = pulseWidthModStart * envelopeStarts[2 /* EnvelopeComputeIndex.pulseWidth */];
                let pulseWidthEnd = pulseWidthModEnd * envelopeEnds[2 /* EnvelopeComputeIndex.pulseWidth */];
            ...
            let expressionStart = settingsExpressionMult * fadeExpressionStart * chordExpressionStart * pitchExpressionStart * envelopeStarts[0 /* EnvelopeComputeIndex.noteVolume */] * supersawExpressionStart;
            let expressionEnd = settingsExpressionMult * fadeExpressionEnd * chordExpressionEnd * pitchExpressionEnd * envelopeEnds[0 /* EnvelopeComputeIndex.noteVolume */] * supersawExpressionEnd;
            // Check for mod-related volume delta
            if (this.isModActive(Config.modulators.dictionary["note volume"].index, channelIndex, tone.instrumentIndex)) {
                // Linear falloff below 0, normal volume formula above 0. Seems to work best for scaling since the normal volume mult formula has a big gap from -25 to -24.
                const startVal = this.getModValue(Config.modulators.dictionary["note volume"].index, channelIndex, tone.instrumentIndex, false);
            ...
            if (instrument.type == 7 /* InstrumentType.pickedString */) {
                ...
                if (tone.prevStringDecay != null) {
                    ...
                    const sustainEnvelopeStart = tone.envelopeComputer.envelopeStarts[3 /* EnvelopeComputeIndex.stringSustain */];
                    stringDecayStart = 1.0 - Math.min(1.0, sustainEnvelopeStart * tone.stringSustainStart / (Config.stringSustainRange - 1));
                }
                const sustainEnvelopeEnd = tone.envelopeComputer.envelopeEnds[3 /* EnvelopeComputeIndex.stringSustain */];
    ...
    static supersawSynth(synth, bufferIndex, runLength, tone, instrumentState) {
        ...
        let supersawFunction = Synth.supersawFunctionCache[0]; //currently only one supersaw function can exist in a given song / mod. Change to an array if you desire to support multiple by, for example, having unisons on supersaws
...
Synth.loopableChipFunctionCache = Array(Config.unisonVoicesMax + 1).fill(undefined); //For loopable chips, we have a matrix where the rows represent voices and the columns represent loop types
