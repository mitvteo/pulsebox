            ...
            case EnvelopeType.pseudorandom:
                ...
                switch (waveform) {
                    case RandomEnvelopeTypes.time:
                        if (step <= 1) return 1;
                        const timeHash: number = XXH((perEnvelopeSpeed == 0 ? 0 : Math.floor((timeSinceStart * perEnvelopeSpeed) / (256))) + "", seed);
                        if (inverse) {
                    ...
                    case RandomEnvelopeTypes.pitch:
                        const pitchHash: number = XXH(defaultPitch + "", seed);
                        if (inverse) {
                    ...
                    case RandomEnvelopeTypes.note:
                        if (step <= 1) return 1;
                        const noteHash: number = XXH(notePinStart + "", seed);
                        if (inverse) {
                    ...
                    case RandomEnvelopeTypes.timeSmooth:
                        const timeHashA: number = XXH((perEnvelopeSpeed == 0 ? 0 : Math.floor((timeSinceStart * perEnvelopeSpeed) / (256))) + "", seed);
                        const timeHashB: number = XXH((perEnvelopeSpeed == 0 ? 0 : Math.floor((timeSinceStart * perEnvelopeSpeed + 256) / (256))) + "", seed);
